        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm" id="tabelPeminjaman">
            <thead class="bg-gray-50 dark:bg-[#0B132B] text-gray-800 dark:text-gray-200 uppercase text-xs tracking-wider">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left">Status Waktu</th>
...
@php
    ...
    $selisih = $terlambat ? $jatuhTempo->diffInDays($hariIni) : 0;
@endphp
<tr id="row-{{ $p->id }}" data-status="{{ $p->status }}" class="hover:bg-gray-50 dark:hover:bg-[#0f1b3d] transition">
                    ...
                    <td class="px-4 py-3">
                        @if ($p->status === 'dikembalikan')
                            <span class="status-badge bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300">
                                📦 Selesai
                            </span>
                        @elseif ($terlambat)
                            <span class="status-badge bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300 animate-pulse">
...
document.querySelectorAll('.btn-kembalikan').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        document.getElementById('mBuku').innerText = btn.dataset.nama;
        document.getElementById('mAnggota').innerText = btn.dataset.anggota;
        document.getElementById('mPinjam').innerText = btn.dataset.pinjam;
        document.getElementById('mKembali').innerText = btn.dataset.kembali;
...
// ✅ Konfirmasi AJAX update status
document.getElementById('konfirmasiBtn').addEventListener('click', async () => {
    if (!currentId) return;

    const url = "{{ route('peminjaman.updateStatus', ':id') }}".replace(':id', currentId);
    ...
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById(`tgl-${currentId}`).innerText = today;
        ...
        showToast('error', '⚠️ Tidak dapat terhubung ke server.');
    }
...
function showToast(type, message) {
    ...
    toast.innerHTML = `
        <div class="${colors[type] || 'bg-gray-700'} text-white px-5 py-3 rounded-xl shadow-lg animate-fade-in">
            ${message}
        </div>`;
